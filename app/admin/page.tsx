...
export default function AddProduct() {
  ...
  // FIX: Default values ensure inputs are never "undefined"
  ...
  const handleSubmit = async () => {
    // 1. Frontend Validation (Prevents the "Description Required" Server Error)
    if (!formData.title || !formData.description) {
        alert("Please enter a Title and Description.");
        return;
    }
    if (!formData.fileUrl || !formData.image) {
        alert("Please upload both a Cover Image and the Product File.");
        return;
    }
